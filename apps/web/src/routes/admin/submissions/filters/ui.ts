...
export type MobileFilterId = ValueFilterId | 'time'
export type VirtualRow = { index: number; size: number; start: number }
...
export function rootFilterOptionKey(match: RootFilterOptionMatch) {
...
export function valueFilterSummary(
  ...
  if (filter.selected.length === 1) {
    ...
    if (selected)
      return family.optionView(selected as ValueFilterOption).textValue
  }
