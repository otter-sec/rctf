<script lang="ts">
  ...
  const hasNextTeamPage = $derived(
    (shouldFetchRegisteredRows && (usersQuery.hasNextPage ?? false)) ||
      (pendingVerificationsQuery.hasNextPage ?? false)
  )
  const isFetchingTeams = $derived(
    (usersQuery.isFetching && !usersQuery.isFetchingNextPage) ||
      (pendingVerificationsQuery.isFetching && !pendingVerificationsQuery.isFetchingNextPage)
  )
  const isFetchingNextTeamPage = $derived(
    usersQuery.isFetchingNextPage || pendingVerificationsQuery.isFetchingNextPage
  )
  ...
  function loadMoreTeams() {
    if (shouldFetchRegisteredRows && usersQuery.hasNextPage && !usersQuery.isFetchingNextPage) {
      usersQuery.fetchNextPage()
    }
    if (pendingVerificationsQuery.hasNextPage && !pendingVerificationsQuery.isFetchingNextPage) {
      pendingVerificationsQuery.fetchNextPage()
    }
  }
