<script lang="ts">
  ...
  import ProfileTeamAvatar from './profile-team-avatar.svelte'
  import ProfileTeamInfo from './profile-team-info.svelte'
  import ProfileTeamMembers from './profile-team-members.svelte'
  ...
  let { user, clientConfig, showMembersSection = false }: Props = $props()
...
<div class="bg-background-l1 flex h-[calc(100vh-72px)] flex-col overflow-hidden rounded-t-3xl">
  ...
  >
    <div class="flex flex-col gap-4 px-4 pt-2 pb-4">
      <ProfileTeamAvatar {user} {clientConfig} />
      <ProfileTeamInfo {user} {clientConfig} />
