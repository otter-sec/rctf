...
type PendingRegistrationPagination = {
  limit: number
  offset: number
}

type PendingRegistrationVerificationList = {
  total: number
  verifications: PendingRegistrationVerification[]
}
...
export const getPendingRegistrationVerifications = async (
  db: DatabaseClient,
  pagination: PendingRegistrationPagination
): Promise<PendingRegistrationVerificationList> => {
  await db
    .delete(pendingUserVerifications)
    .where(sql`${pendingUserVerifications.expiresAt} <= now()`)

  const [countResult, verifications] = await Promise.all([
    db
      .select({ count: count() })
      .from(pendingUserVerifications)
