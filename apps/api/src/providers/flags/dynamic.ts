...
export const verifyDynamicFlag = (
  ...
  if (
    ...
    return FlagVerifyStatus.ACCEPTED
  }
  ...
  switch (signingMode) {
    ...
    case DynamicFlagMode.BASIC:
      ...
      let submittedFlagBase = submittedFlagContentChunks.slice(0, -2).join(':')

      if (submittedFlagBase !== baseFlagContent) {
        return FlagVerifyStatus.REJECTED
      }
      break
  }
...
const signingKey = (): string => serverConfig.dynamicFlagSigningKey ?? ''
