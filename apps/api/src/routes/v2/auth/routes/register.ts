...
authGroup.route(RegisterRouteV2, async ({ res, body, ctx }) => {
  ...
  if (body.discordToken && !config.discord) {
    return res.badEndpoint()
  }
  ...
  const userToCreate: Pick<User, 'division' | 'email' | 'name' | 'ctftimeId' | 'discordId'> =
    {
      division,
      email: body.email,
      name: body.name,
  ...
  if (body.discordToken) {
    ...
    userToCreate.discordId = discordToken.discordId
  }
