...
<Callout variant="warning" title="v2 needs delete permissions">
  ...
  v1 IAM policy that only grants `<green>s3:GetObject</green>` / `<green>s3:PutObject</green>`,
  add `<green>s3:DeleteObject</green>` to it (or the GCS equivalent `<green>storage.objects.delete</green>`).
</Callout>
