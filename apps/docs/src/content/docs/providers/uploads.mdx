...
- Files are stored by their SHA256 hash, so uploading the same file content twice does not create a duplicate.
- All uploaded files are served with immutable cache headers (`max-age=31536000`).
- Team avatars are resized to 256x256 pixels and converted to WebP format before upload. The maximum avatar size is controlled by the `<red>maxAvatarSize</red>` config option (default 1 MB).
