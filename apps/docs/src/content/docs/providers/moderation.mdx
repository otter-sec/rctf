...
| ----------------------------------------------- | -------------- | ----------- | ------------------------------------------------------------- |
| `avatarsModeration.provider{:yaml}`             | `object{:ts}`  | -           | Moderation provider configuration                             |
| `avatarsModeration.allowOnInternalError{:yaml}` | `boolean{:ts}` | `true{:ts}` | Whether to allow avatar uploads when the moderation API fails |

<Callout variant="tip">

Setting `allowOnInternalError{:yaml}` to `true{:ts}` (the default) ensures that temporary OpenAI API outages do not prevent users from uploading avatars. Set to `false{:ts}` if strict moderation is required.
...
| --------------- | -------------------------------------------------------------- | ------------------------ | ----------------------- |
| `apiKey{:yaml}` | `RCTF_MODERATION_OPENAI_API_KEY{:sh}` or `OPENAI_API_KEY{:sh}` | -                        | OpenAI API key          |
| `model{:yaml}`  | `RCTF_MODERATION_OPENAI_MODEL{:sh}`                            | `omni-moderation-latest` | OpenAI moderation model |
...
1. Image is validated against `maxAvatarSize{:yaml}` (default 1 MB)
