...
```ansi
$ [31mbunx[0m [32mshadcn-svelte@latest[0m add button
```

This creates the component files in `apps/web/src/lib/components/ui/button/`.

</Step>
<Step title="Register in the barrel file">
...
<Step title="Normalize font weights">

Convert all heavy font weights to `font-medium` for visual consistency:

| Original                 | Replace with  |
| ------------------------ | ------------- |
| Any weight above medium  | `font-medium` |

</Step>
<Step title="Convert color tokens">
...
<Tabs items={['Before (shadcn-svelte)', 'After (rCTF)']}>
<Tab>
