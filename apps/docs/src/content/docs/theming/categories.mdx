...
Each challenge category in rCTF has a dedicated color palette, icon, and display name configured in `apps/web/src/lib/utils/categories.ts{:file}`. The system uses dynamic CSS variable injection to apply category-specific colors to components. A category's color key (e.g., `red`) maps to CSS variables like `--category-foreground-l0`, which components reference via Tailwind classes such as `text-category-foreground-l0`.
...
Each color has five variables defined in `app.css{:file}` for both light and dark modes: `background-l0`, `background-l1`, `background-l1-hover`, `foreground-l0`, and `foreground-l1`.
...
If your category uses a color not already in the palette, add variables to `app.css{:file}` following the existing pattern. Define `--background-{color}-l0`, `--background-{color}-l1`, `--background-{color}-l1-hover`, `--foreground-{color}-l0`, and `--foreground-{color}-l1` for both `:root` and `[data-theme='dark']`. Don't forget to additionally map the colors in `@theme inline`.
