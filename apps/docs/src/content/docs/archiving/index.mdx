...
| `--web-build` | `apps/web/build/{:dir}` | Pre-built SvelteKit frontend that is copied into the output. |
| `--output` | `./export-output/{:dir}` | Destination directory. Wiped and recreated on every run. |
...
Publish with `gh-pages` from the repository root. The `--dotfiles` flag is required so `.nojekyll{:file}` is uploaded:
