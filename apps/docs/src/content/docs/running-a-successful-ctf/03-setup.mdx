...
```ansi
$ [31mssh[0m [32mroot@123.123.123.123[0m
```
...
- `[31mproxy.cloudflare[0m{:ansi}` if behind Cloudflare
...
```ansi
$ [31mufw[0m allow 22
[02m# source: https://github.com/Paul-Reed/cloudflare-ufw/blob/master/cloudflare-ufw.sh[0m
$ [31mfor[0m cfip in `[31mcurl[0m [02m-sw[0m [32m'\n'[0m [32mhttps://www.cloudflare.com/ips-v{4,6}[0m`; do [31mufw[0m allow proto tcp from $cfip comment [32m'Cloudflare IP'[0m; done
$ [31mufw[0m enable   [02m# answer `y` when prompted regarding enabling firewall[0m
```

</Step>
<Step title="Install and configure Nginx">

Install Nginx using `apt update && apt install -y nginx`. Then, create `/etc/nginx/sites-available/rctf.conf` with the following contents (note the comments on stuff you need to update):
...
```ansi
$ [31mln[0m [02m-s[0m /etc/nginx/sites-available/rctf.conf /etc/nginx/sites-enabled/rctf.conf
$ [31mnginx[0m [02m-t[0m && [31mnginx[0m [02m-s[0m reload
```
