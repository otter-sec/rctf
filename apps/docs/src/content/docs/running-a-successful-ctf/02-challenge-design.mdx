...
If you are using rCTF's [instancer integration](/integrations/instancer), the Docker Compose-like configuration is defined in the challenge's `[31minstancerConfig[0m{:ansi}` field. The instancer handles container lifecycle, networking, and cleanup automatically.
