...
<Callout variant="warning" title="Hostile workloads">
  ...
  wider RBAC, or any other "sensitive" config changes can quickly break that assumption.
</Callout>
...
| --- | --- | --- |
| `<red>rctf_instancer_api_url</red>` | `<red>instancers.k8s.options.apiUrl</red>` | `K8S_INSTANCER_API_URL{:sh}` |
| `<red>rctf_instancer_auth_token</red>` | `<red>instancers.k8s.options.authToken</red>` | `K8S_INSTANCER_AUTH_TOKEN{:sh}` |
| `<red>rctf_instancer_ca_certificate</red>` | `<red>instancers.k8s.options.caCertificate</red>` | `K8S_INSTANCER_CA_CERTIFICATE{:sh}` |
