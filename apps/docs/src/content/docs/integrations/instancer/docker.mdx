...
Every instanced challenge creates its own user-defined Docker network. Docker carves those networks out of the daemon's default address pool, which on a fresh install is only `172.17.0.0/16` plus a small chunk of `192.168.0.0/16` - enough for ~30 networks before `$ <red>docker</red> network create` starts failing with `could not find an available, non-overlapping IPv4 address pool`. For a production CTF this runs out fast.
...
Restart Docker after editing the file (`$ <red>systemctl</red> restart docker`) so the new pool is picked up. Networks already created on the old pool keep working but consume their original ranges.
