...
<Tabs items={['Docker', 'Kubernetes', 'Paradigmctf.py']}>
<Tab>
...
`<green>instancer/paradigmctf-instancer</green>` talks to a custom [paradigmctf-py](https://github.com/es3n1n/paradigmctf-py) server running in API mode, for EVM-style blockchain challenges. Define it once with no options:

```yaml title="rctf.d/instancer.yaml"
instancers:
  paradigm:
    name: instancer/paradigmctf-instancer
defaultInstancer: paradigm
```

The connection details live on the challenge instead of the provider, so a single entry serves every paradigm challenge. Each challenge sets its own API URL in `<red>config.apiUrl</red>`, and because the provider returns the per-team RPC endpoints, player private key, and deployed contract addresses as RAW results, `<red>expose</red>` stays empty:
...
instancerConfig:
  ...
  config:
    apiUrl: http://my-evm-chall:1337
  expose: []
```

| Field                      | Purpose                                                     |
| -------------------------- | ----------------------------------------------------------- |
| `<red>config.apiUrl</red>` | Base URL of the challenge's paradigmctf-py API-mode server. |
...
`<green>raw</green>` results are produced by providers for connection info that has no host or port. rCTF shows the text with a copy button.
