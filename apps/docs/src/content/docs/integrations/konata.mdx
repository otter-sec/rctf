...
When `true{:ts}` (the default), every archive Konata builds during `sync` puts its contents under a single top-level folder named after the archive, so `pwn_garden.tar.gz{:file}` extracts into `pwn_garden/{:dir}` instead of dumping files into whatever directory the player happens to be in. Set it to `false{:ts}` to keep entries at the archive root. Pre-compressed attachments are uploaded as-is either way.

## Per-challenge config

Each challenge directory has a `kona.yml{:file}` (or `kona.yaml{:file}` / `kona.toml{:file}`) that declares one or more challenges plus an optional `<red>deployment</red>` block. The simplest static challenge needs nothing but a category, name, author, description, and flag:
...
| `<red>tags</red>` | Tag strings shown to players on the challenge page. |
...
| `<red>admin_bot</red>` | Admin bot handler source for web challenges (see [Admin bot](#admin-bot)). |
...
| `<red>archive_name</red>` | Base name of the generated archive (and of the folder it extracts into). Defaults to the challenge ID. Characters that can't appear in a Windows filename are replaced with `_`. |
| `<red>strip_components</red>` | Leading path components dropped from `<red>files</red>` entries inside the archive, like `tar <dim>--strip-components</dim>`. With `1{:ts}`, `dist/bzImage{:file}` is stored as `bzImage{:file}`. If stripping would eat the whole path, the entry keeps its filename. |

Resolution is strict: a `<red>files</red>` entry that doesn't exist, a glob that matches nothing, or a `<red>pre_compressed</red>` path with no archive behind it fails the sync. Konata won't quietly ship an incomplete handout.
...
Web challenges that use the [rCTF admin bot](/integrations/admin-bot/) can ship their handler source in the same sync. `<red>admin_bot</red>` takes exactly one of:

| Field             | Purpose                                                |
| ----------------- | ------------------------------------------------------ |
| `<red>code</red>` | Inline TypeScript source.                              |
| `<red>file</red>` | Source file path, relative to the challenge directory. |

The source is rendered with Jinja like everything else (`<red>challenge</red>`, `<red>config</red>`, and `<red>images</red>` are in scope), plus a `re_escape` filter for splicing values into regex patterns:

```yaml title="web/safebox/kona.yml"
challenges:
  - category: web
    name: safebox
    # ...
    admin_bot:
      code: |
        import { Challenge, type ChallengeContext } from 'types'

        export const challenge = new Challenge({
          timeoutMilliseconds: 30_000,
          inputs: {
            url: { pattern: '^https://{{ challenge.name }}\\.{{ config.domains["kctf"] | re_escape }}/' },
          },
          handler: async (ctx: ChallengeContext) => {
...
Anything longer than a few lines reads better as a separate file:
...
Both forms go through the template pass. If the handler itself needs a literal `{{` or `{%` (client-side template injection payloads, for example), wrap that part in `{% raw %}` / `{% endraw %}` so Jinja leaves it alone.
...
Builds run through `$ <red>docker</red> buildx build <dim>--pull</dim>`, so BuildKit-only Dockerfile syntax (cache mounts, heredocs) works fine, and `<red>exports</red>` come straight out of the build via `<dim>--output type=local</dim>`. The `docker` CLI with the buildx plugin has to be on `PATH` wherever Konata runs.
