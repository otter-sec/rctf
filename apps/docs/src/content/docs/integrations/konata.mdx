...
[Konata](https://github.com/project-sekai-ctf/konata) (CLI command: `kona`) is a CTF challenge-management tool. It walks a repository of challenge directories, builds and pushes Docker images, applies Kubernetes manifests, and syncs the resulting challenge metadata to rCTF (and CTFd, when configured) - all driven by per-challenge `kona.yml` files and a single root `kona.yml` for global settings.

This page covers Konata as it relates to rCTF. For the upstream tool and source see the [Konata repository](https://github.com/project-sekai-ctf/konata).

<Callout variant='note' title='Pre-1.0 status'>
  Konata is pre-1.0. The schema is settled enough that we've used it to run public CTFs end-to-end
  (the DiceCTF Quals 2026 challenge repository is a good real-world reference) but breaking changes
  are still on the table until 1.0.
</Callout>
...
Python 3.12 or newer is required. The `kona` binary exposes two commands: `<black>sync</black>` (the main workflow) and `<black>compress</black>` (a helper for packaging attachment folders ahead of time).

## Repository layout

Konata assumes a directory tree with one root config at `kona.yml` and one config per challenge somewhere underneath it. The challenge folder structure is arbitrary; Konata walks up to `<red>discovery.challenge_folder_depth</red>` levels deep (default `3{:ts}`) looking for files named `kona.yml` or `kona.yaml`.

A typical layout (taken from the DiceCTF Quals 2026 deployment repository):

<FileTree>
  <Dir name='ctf-challenges'>
    <File name='kona.yml' highlight comment='Global config' />
    <Dir name='web'>
      <Dir name='dicewallet'>
        <File name='kona.yml' comment='Per-challenge config' />
        <File name='Dockerfile' />
        <Dir name='challenge' />
      </Dir>
      <Dir name='mirror-temple'>
        <File name='kona.yml' comment='Per-challenge config' />
      </Dir>
    </Dir>
    <Dir name='pwn'>
    ...
    <Dir name='sanity'>
      <Dir name='survey'>
        <File name='kona.yml' comment='Skipped because discovery.skip: true' />
      </Dir>
    </Dir>
  </Dir>
</FileTree>
...
The root `kona.yml` configures credentials, clusters, registries, and shared template overrides. Everything below is optional; a minimum config just needs an `<red>rctf</red>` block.
...
Named references the rest of the config can pull from. Each entry must specify exactly one of:

| Field | Behavior |
| --- | --- |
| `<red>value</red>` | Inline literal. Useful for non-secret values you still want to centralize. |
| `<red>file_path</red>` | Path read from disk at load time. Relative paths resolve against the directory containing the root `kona.yml`. |
...
rCTF API credentials. Konata talks to rCTF over the public admin API.
...
A `<red>ctfd</red>` block with the same shape exists for CTFd deployments; both can coexist if you mirror the same challenge repo to multiple platforms.
...
| Backend | Use when |
| --- | --- |
| `<red>gcloud</red>` | Targeting GKE. Konata runs `gcloud container clusters get-credentials` under the hood - the workflow needs a logged-in `gcloud` and the `gke-gcloud-auth-plugin`. |
...
| `<red>use_default: true</red>` | Use `$KUBECONFIG` or `~/.kube/config` from the host (the default). |
| `<red>incluster: true</red>` | Use the in-pod service-account credentials when Konata itself runs inside the cluster. |

`<red>alias_to</red>` lets a challenge reference one cluster name while routing to another. The DiceCTF example uses this so per-challenge configs say `<red>cluster_name: main</red>` and the operator swaps `main → prod` at the root level.

### `registries`

Aliases for container registry prefixes. A challenge's `<red>deployment.images[].registry_name</red>` looks up an entry here and prepends the resolved value to the image name. Splitting registries (one for static-hosted images, one for instancer images) is common when the rCTF instancer cluster and the kCTF cluster run in different GCP projects.

### `domains`

Free-form key/value map of domain names available to the Jinja templates inside challenge configs (`{{ config.domains['kctf'] }}` in the examples). Useful for keeping the deploy domain out of every per-challenge config.

### `templates`

Overrides for the rendered challenge description and endpoint block. The defaults wrap the description with a connection-info section followed by `**Author**: ...`. Overrides are Jinja2 templates with `<red>challenge</red>`, `<red>config</red>`, and `<red>models</red>` in scope.
...
| `<red>endpoints_text.rctf</red>` | Endpoints template used when syncing to rCTF. The default renders a `> [!CONNECTION]` callout, which the rCTF frontend turns into a styled connection-info box. |
...
`<red>endpoints_text</red>` is per-provider because rCTF and CTFd render markdown differently: each provider sees its own rendered endpoints, then `<red>challenge_description</red>` is run once per provider with that provider's `<red>endpoints_rendered</red>` in scope. To override just one side, set only that key - the other keeps its default.
...
Top-level discovery knobs:

| Field | Default | Purpose |
| --- | --- | --- |
| `<red>challenge_folder_depth</red>` | `3{:ts}` | Max depth from root when scanning for `kona.yml` / `kona.yaml`. |
...
`<green>tar_gz</green>` (default) or `<green>zip</green>`. Controls the archive format Konata uses when bundling attachment files for upload.

## Per-challenge config

Each challenge directory has a `kona.yml` (or `kona.yaml`) that declares one or more challenges plus an optional `<red>deployment</red>` block. The minimum static challenge has nothing but a category, name, author, description, and flag:
...
`<red>discovery.skip: true</red>` opts the directory out of discovery - useful when the file is committed but the challenge is not yet ready to deploy.
...
| Field | Purpose |
| --- | --- |
| `<red>files</red>` | Files relative to the challenge directory. Directory entries (`chall/src/`) recurse into all files underneath. |
| `<red>exclude</red>` | Globs filtered out of the resolved file list before archiving. |
| `<red>additional</red>` | Synthetic files injected into the archive. Each entry specifies `<red>path</red>` plus exactly one of `<red>str</red>` or `<red>base64</red>`. Typical use: a dummy flag file so the build still works. |
...
`<red>flags</red>` is a per-platform map. Each platform has its own shape - rCTF accepts exactly one flag, CTFd accepts a list - and within each platform a flag value can be written in three forms: an inline literal string, `<red>{ str: ... }</red>`, or `<red>{ file: ... }</red>` (read at sync time, path resolved relative to the challenge directory).

```yaml
# rCTF flag, inline literal - most common.
flags:
  rctf: dice{example}
...
Static-hosting deployments declare `<red>endpoints</red>` so Konata can render connection info into the description. Each entry has a `<red>type</red>` (one of `<green>http</green>`, `<green>https</green>`, `<green>socat</green>`, `<green>nc</green>`, `<green>ncat-ssl</green>`), an `<red>endpoint</red>` host, and an optional `<red>port</red>`. Jinja templating is available on `<red>endpoint</red>` so the host can be derived from the challenge name and the global `<red>domains</red>` map:
...
`<red>deployment</red>` declares what Konata should build and what manifests to apply on top of the challenge sync. Both blocks are optional - a static challenge with just a flag and an attachment skips deployment entirely.
...
| `<red>build_args</red>` | Build-time `<black>--build-arg</black>` values. |
...
Image references inside Kubernetes manifests can interpolate `{{ images[challenge.name] }}` to get the fully-resolved `registry/name:tag` for the current challenge.
...
| `<red>paths</red>` / `<red>documents</red>` | YAML files on disk or inline objects. Both are rendered as Jinja templates with `<red>challenge</red>`, `<red>challenges</red>`, `<red>images</red>`, and `<red>config</red>` in scope. |
...
The instanced flow only needs pod definitions - the rCTF instancer's k8s-controller handles namespaces, services, network policies, and ingress at runtime. No `<red>kubernetes_inline_manifests</red>` block is required.

## CLI

### `kona sync`

The main entry point. Walks the deploy directory, builds and pushes any declared images, applies Kubernetes manifests, and syncs challenge metadata to every configured platform.
...
| Flag | Behavior |
| --- | --- |
| `-d`, `--deploy-directory` | Root of the deploy repo (the folder containing the root `kona.yml`). Required. |
| `--only <name>` | Repeatable. Restricts the run to specific challenge folder names. Discovery still walks the tree; non-matching challenges are skipped. |
...
| `-f`, `--format`    | `<green>tar_gz</green>` (default) or `<green>zip</green>`.              |
| `-o`, `--output`    | Output path. Defaults to `<src>.{tar.gz,zip}` in the current directory. |

Both formats are **deterministic**: file mtimes are zeroed (fixed to `1980-01-01` for zip, `0` for tar.gz), uid/gid become `0`, uname/gname become `<green>kona</green>`, permissions are normalized to `0777`, the gzip header drops its own filename and mtime, and entries are sorted by archive path. Compressing the same set of files always produces a byte-identical archive (and therefore an identical hash), so attachment dedup and CI cache hits stay stable across machines, runs, and users. The same normalization is applied to attachments Konata builds internally during `<black>sync</black>`.

## CI integration

The [`project-sekai-ctf/konata-deploy-action`](https://github.com/project-sekai-ctf/konata-deploy-action) GitHub Action exposes two subactions: `detect` walks the repository and emits a matrix of changed challenges, and `sync` runs `kona sync --challenge-path <one>` for one matrix shard. Together they restrict each push to redeploying only the challenges that changed.
...
- **`RCTF_TOKEN`** is the admin team token referenced by the root `kona.yml` (`<red>secrets.token.env: RCTF_TOKEN</red>`).
