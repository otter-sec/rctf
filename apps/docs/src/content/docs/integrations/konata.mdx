...
`<red>discovery.skip: true</red>` opts the directory out of discovery, which is useful when the file is committed but the challenge is not yet ready to deploy.
...
`<red>flags</red>` is a per-platform map. Each platform has its own shape, where rCTF accepts exactly one flag and CTFd accepts a list. Within each platform, a flag value can be written in three forms. The forms are an inline literal string, `<red>{ str: ... }</red>`, or `<red>{ file: ... }</red>` (read at sync time, with the path resolved relative to the challenge directory).
