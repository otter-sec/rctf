...
Python 3.12 or newer is required. The `$ <red>kona</red>` binary exposes two commands: `sync` (the main workflow) and `compress` (a helper for packaging attachment folders ahead of time).
...
| Backend | Use when |
| --- | --- |
| `<red>gcloud</red>` | Targeting GKE. Konata runs `$ <red>gcloud</red> container clusters get-credentials` under the hood - the workflow needs a logged-in `$ <red>gcloud</red>` and the `gke-gcloud-auth-plugin`. |
...
| `<dim>--only</dim> <name>` | Repeatable. Restricts the run to specific challenge folder names. Discovery still walks the tree; non-matching challenges are skipped. |
