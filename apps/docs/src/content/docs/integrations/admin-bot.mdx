...
Admin bot jobs are throughput-bound on browser sessions, not CPU. A worker spends most of its wall time waiting on Puppeteer - page loads, redirects, and the configured timeout - so steady-state CPU on a worker can stay low even when the queue is backing up. Autoscaling on CPU will react late and under-provision; scaling on **queue depth** is the right signal.

The `<route>GET /api/v2/admin/admin-bot/queue-depth</route>` endpoint returns the number of queued jobs and is meant to drive that signal. A practical setup is to scrape it (via an exporter or a small sidecar) into Prometheus, set a target queue depth per replica (e.g. 5–10), and let a KEDA `ScaledObject` or HPA `External` metric scale workers up and back down.
