...
<Callout variant='warning' title='Not battle-tested under high load'>
  Most of rCTF v2 has run large public events, but the admin bot hasn't been pushed under that kind
  of traffic yet. The implementation works and we expect it to hold up, but expect rougher edges
  than the rest of the platform. Load-test before relying on it for a high-volume event, and watch
  the queue depth and worker logs during the CTF.
...
export const challenge = new Challenge({
  ...
  // optional, everything defaults to off and no tab limit:
  hooksConfig: {
...
| `hooksConfig{:ts}` | No | What the browser hooks log, and the tab limit. You can skip fields or the whole object; everything defaults to off. |
...
| `showBrowserErrors{:ts}` | Captures page errors, failed network requests, and service worker attach failures. |
| `showNavigation{:ts}` | Captures tab creation, main-frame navigation, extension page creation, service worker creation, and tab close events. |
| `showDialogs{:ts}` | Captures the type and message of `alert{:ts}`, `confirm{:ts}`, `prompt{:ts}`, and `beforeunload{:ts}` dialogs. |
| `autoDismissDialogs{:ts}` | Dismisses (cancels) JavaScript dialogs so an unhandled `alert(){:ts}` or `confirm(){:ts}` can't block the page. Yields to a dialog your handler already accepted or dismissed. |
...
| `limitTabsNumberShowError{:ts}` | Writes the `tab limit exceeded` error to participant logs when the limit trips. Leave it off if that message would hand participants a side channel, since it confirms their payload opened tabs. |

Every boolean here defaults to `false{:ts}` and `limitTabsNumber{:ts}` defaults to `-1{:ts}`, so an omitted `hooksConfig{:ts}` means no automatic logging and no tab limit.

<Callout variant="warning" title="Firefox hook coverage">
  Firefox doesn't support extension page logging and service worker console logging in the current
  worker.
</Callout>
