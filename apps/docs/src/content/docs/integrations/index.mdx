...
| [Instancer](/integrations/instancer/) | `<red>instancers</red>` | Per-team challenge instance management via Docker or Kubernetes. Define multiple instancers and let each challenge pick one. Isolated environments per team with configurable timeouts, manual extension, and automatic cleanup. |
