...
Puppeteer-based browser automation for testing web challenges. Challenge authors write JavaScript code that runs in a browser context, useful for XSS and web exploitation challenges. Configured via the `adminBot{:yaml}` config section and managed through the admin API.
