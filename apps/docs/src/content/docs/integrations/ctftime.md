...
5. The API exchanges the code for a CTFtime access token and creates an rCTF authentication token
...
Returning users provide their CTFtime token to log in. The API matches their CTFtime ID to an existing account.
...
See [After the CTF](/running-a-successful-ctf/06-after-ctf) for the full scoreboard submission procedure.
