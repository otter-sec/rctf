...
| `<red>idleTimeout</red>` | `number{:ts}` | `65{:ts}` | How long in seconds an idle connection stays open before the server closes it. Set it higher than your reverse proxy's keep-alive timeout. Capped at `255{:ts}` (Bun's limit); `0{:ts}` disables the timeout. |
