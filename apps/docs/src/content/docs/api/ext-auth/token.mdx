...
description: '<route>POST /api/v2/ext-auth/token</route>'
---

import Callout from '@/components/callout.astro'
import { BadBody, BadExtAuthRequest, BadJson, ExtAuthTokenRouteV2 } from '@rctf/types'

<RouteAside>
  <RouteExample
    def={ExtAuthTokenRouteV2}
    body={{
...
<RouteMetaFromDef def={ExtAuthTokenRouteV2} />
...
<Callout variant="note" title="Failures all look identical">
  ...
  return the same `<response>400 badExtAuthRequest</response>` body. Don't rely on the failure
  reason to debug an integration - check timestamps, secret rotation, and the exact bytes you sent
  instead.
</Callout>

<RequestBody def={ExtAuthTokenRouteV2} title="Request body" />

<ResponseBody def={ExtAuthTokenRouteV2} response="goodExtAuthToken" title="Response fields" />
