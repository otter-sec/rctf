...
The versions are route contracts, not separate complete APIs. Use V2 where a V2 route exists, and use V1 routes for actions that do not have V2 replacements. Many V2 routes share the same response `kind` string as their V1 equivalents, but the `data` payload is often wider in V2 (avatars, country codes, blood index, etc.).
...
```text
Authorization: Bearer <dim><auth-token></dim>
```
