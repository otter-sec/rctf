...
import { BadBody, BadJson, BadSignature, SubmitDynamicScoresRouteV2 } from '@rctf/types'

<RouteAside>
  <RouteExample
    def={SubmitDynamicScoresRouteV2}
    ...
    extra={[BadJson, BadBody, BadSignature]}
  />
</RouteAside>

<RouteMetaFromDef def={SubmitDynamicScoresRouteV2} />
...
<Callout variant="warning" title="No event-timing gate">
...
<RequestBody def={SubmitDynamicScoresRouteV2} source="params" title="Path parameters" />

<RequestBody def={SubmitDynamicScoresRouteV2} title="Request body" />
