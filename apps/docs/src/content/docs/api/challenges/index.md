...
Challenge routes provide public challenge metadata, solve history, the legacy flag submission endpoint, and the dynamic-scoring webhook. Admin challenge mutation routes are documented in [Admin](/docs/api/admin/).
...
Challenge point values come from the configured [scoring provider](/docs/providers/scores/) for `decay` challenges. The public challenge list returns the current point value. It does not expose the configured `points.min` or `points.max` range. Challenge updates and solve changes trigger leaderboard recalculation.

`dynamic` challenges receive per-team scores over a webhook instead - see [Submit dynamic scores](/docs/api/challenges/submit-dynamic-scores/) for the wire format and [Scoring](/docs/admin/scoring/) for the operator-facing model.
