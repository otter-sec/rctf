...
<RouteAside>
  <RouteList>
    <RouteListItem
      ...
      method="GET"
      path="/api/v2/external-auth/clients/:id"
      href="/api/external-auth/get-client/"
    />
    <RouteListItem
      title="Authorize"
      method="POST"
      path="/api/v2/external-auth/authorize"
      href="/api/external-auth/authorize/"
    />
    <RouteListItem
      title="Exchange code for token"
      method="POST"
      path="/api/v2/external-auth/token"
      href="/api/external-auth/token/"
    />
  </RouteList>
</RouteAside>
