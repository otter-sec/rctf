...
| `<response>400 badExtAuthRequest</response>` | No data | An [external-auth](/api/ext-auth/) call was rejected. The same response is used for unknown client, wrong secret, wrong/expired/reused code, and mismatched redirect URI so the endpoint can't be probed. |
...
| `<response>200 goodExtAuthClient</response>` | `{ id: string, name: string, redirectUri: string }{:ts}` |
| `<response>200 goodExtAuthAuthorize</response>` | `{ redirectTo: string }{:ts}` |
| `<response>200 goodExtAuthToken</response>` | `{ accessToken: string, tokenType: "bearer" }{:ts}` |
| `<response>200 goodAdminExtAuthClients</response>` | `{ id, name, redirectUri, createdAt, createdBy }[]{:ts}` |
| `<response>200 goodAdminExtAuthClientCreate</response>` | `{ id, name, redirectUri, createdAt, createdBy, secret }{:ts}` (secret shown once) |
| `<response>200 goodAdminExtAuthClientDelete</response>` | No data. |
