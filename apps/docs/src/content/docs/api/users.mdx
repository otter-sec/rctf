...
- **Verification tokens** expire after `loginTimeout{:yaml}` (default 1 hour) and are stored in Redis
- **CTFtime tokens** expire after `loginTimeout{:yaml}`

All tokens are encrypted with AES-GCM using the `tokenKey{:yaml}` from the configuration. If `tokenKey{:yaml}` changes, all existing tokens become invalid.
...
1. The image is validated against `maxAvatarSize{:yaml}` (default 1 MB)
...
When `userMembers{:yaml}` is enabled (default), teams can add members by email. This is informational - members share the same team token and it does not create separate accounts. The maximum number of members is controlled by `maxMembers{:yaml}` (default `50{:ts}`).
