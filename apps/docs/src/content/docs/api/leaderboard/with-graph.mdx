...
<RouteAside>
  <RouteExample def={GetLeaderboardWithGraphRoute} query={{ limit: 50, offset: 0 }} />
</RouteAside>

<RouteMetaFromDef
  def={GetLeaderboardWithGraphRoute}
...
<RequestBody def={GetLeaderboardWithGraphRoute} source="query" title="Query parameters" />

<ResponseBody
  def={GetLeaderboardWithGraphRoute}
  response="goodLeaderboardWithGraph"
