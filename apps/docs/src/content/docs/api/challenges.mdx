...
Each challenge defines a point range with `[31mpoints.min[0m{:ansi}` and `[31mpoints.max[0m{:ansi}`. The current score starts at `[31mmaxPoints[0m{:ansi}` when no one has solved the challenge and decreases toward `[31mminPoints[0m{:ansi}` as more teams solve it. The exact curve depends on the configured [scoring provider](/providers/scores).
...
- **Hidden** - When `[31mhidden[0m{:ansi}` is set to `true{:ts}`, the challenge is completely invisible to non-admin users
- **Release time** - When `[31mreleaseTime[0m{:ansi}` is set to a Unix millisecond timestamp, the challenge becomes visible only after that time. Set to `null{:ts}` to make it visible immediately when the CTF starts.
