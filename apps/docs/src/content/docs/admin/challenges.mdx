...
- **`hidden{:yaml}`** set to **`true{:ts}`** - Challenge is completely hidden from non-admin users
- **`releaseTime{:yaml}`** - Challenge becomes visible only after this timestamp (Unix milliseconds). Set to `null{:ts}` to make it visible immediately when the CTF starts.
...
- **`instancerConfig{:yaml}`** - Configuration for per-team instances. When set, the configuration is validated against the instancer provider's schema. See [Instancer](/integrations/instancer) for details.
- **`adminBotConfig{:yaml}`** - JavaScript code for admin bot testing. The code defines inputs (with regex validation) and a timeout. The admin bot provider validates the configuration when saving.
