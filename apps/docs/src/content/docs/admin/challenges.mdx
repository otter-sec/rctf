...
- **`[31mhidden[0m{:ansi}`** set to **`true{:ts}`** - Challenge is completely hidden from non-admin users
- **`[31mreleaseTime[0m{:ansi}`** - Challenge becomes visible only after this timestamp (Unix milliseconds). Set to `null{:ts}` to make it visible immediately when the CTF starts.
...
- **`[31minstancerConfig[0m{:ansi}`** - Configuration for per-team instances. When set, the configuration is validated against the instancer provider's schema. See [Instancer](/integrations/instancer) for details.
- **`[31madminBotConfig[0m{:ansi}`** - JavaScript code for admin bot testing. The code defines inputs (with regex validation) and a timeout. The admin bot provider validates the configuration when saving.
