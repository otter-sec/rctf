...
rCTF renders Markdown in challenge descriptions, the home page content, and sponsor descriptions. Rendering itself is handled by [`marked`](https://marked.js.org/) with two custom extensions (alerts and timer). The rendered output is then passed through [DOMPurify](https://github.com/cure53/DOMPurify) for sanitization before it reaches the page.
...
rCTF supports GitHub-style blockquote alerts. The opening line is a blockquote containing `[!TYPE]{:md}`, and any subsequent blockquote lines form the alert body. The body is parsed as Markdown (except for `CONNECTION`, see below).
...
`> [!CONNECTION]{:md}` is a real, distinct alert type and not just a styled `NOTE`. It is designed for the **Remote** field of a challenge, which holds a single line containing a connection string or URL.
...
- The only custom attributes preserved beyond defaults are `data-alert`, `data-type`, `data-content`, and `data-timer`, which the renderer uses as hydration markers for the alert and timer extensions. Authoring these attributes by hand has no effect, since they are only emitted by the extensions.
