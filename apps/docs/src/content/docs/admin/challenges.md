...
- **`hidden: true`** - Challenge is completely hidden from non-admin users
- **`releaseTime`** - Challenge becomes visible only after this timestamp (Unix milliseconds). Set to `null` to make it visible immediately when the CTF starts.
...
- **`instancerConfig`** - Configuration for per-team instances. When set, the configuration is validated against the instancer provider's schema. See [Instancer](/integrations/instancer) for details.
- **`adminBotConfig`** - JavaScript code for admin bot testing. The code defines inputs (with regex validation) and a timeout. The admin bot provider validates the configuration when saving.
