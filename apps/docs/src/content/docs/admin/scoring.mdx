...
- Wire a "Sign in with rCTF" button on the dynamic service through [External apps](/admin/ext-auth/) and read the team ID from `<route>/api/v1/users/me</route>` once the user lands back authorized. This is usually the lowest-friction option when teams self-onboard.
