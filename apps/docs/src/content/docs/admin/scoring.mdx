...
<Callout variant="warning" title="Stop the feed when the event ends">
  ...
  still the operator's responsibility. Every late delivery (post-end as well as post-freeze) lands
  in the tally, so a backend that keeps publishing will keep moving the scoreboard around after the
  CTF is supposed to be over.
</Callout>
...
The route accepts any IP and doesn't require an authenticated rCTF user. Only the HMAC and the timestamp window stand between the open internet and `<red>upsertDynamicSolves</red>`, so the secret is the only thing protecting the scoreboard. Treat it like a password.
...
<Callout variant="warning" title="Sign the raw body, not the parsed body">
  ...
  re-serialize the body between signing and sending will cause verification to fail.
</Callout>
