...
<Callout variant="note" title="Retries are safe">
  If a push times out or fails, send the exact same signed request again. rCTF only remembers
  accepted deliveries (for about ten minutes, per challenge, keyed on the signature), so a failed
  attempt never blocks the retry. A `409` `<green>badReplayedRequest</green>` on the retry means the
  first attempt landed after all; count it as a success. Run retries one at a time, and sign every
  new score update with a fresh timestamp instead of reusing an old signature.
</Callout>
