...
- **Read-only filesystem** - Where possible, mount the root filesystem as read-only (`readOnly: true{:yml}` or `<dim>--read-only</dim>`) and use a tmpfs for writable directories.

#### Security defaults

When deploying challenges, apply the following security settings:

- `cap_drop: ALL{:yml}` - Drop all Linux capabilities
- `no-new-privileges: true{:yml}` - Prevent privilege escalation
...
If you are using rCTF's [docker instancer integration](/integrations/instancer/), the Docker Compose-like configuration is defined in the challenge's `<red>instancerConfig</red>` field. The instancer handles container lifecycle, networking, and cleanup automatically.
