---
title: Running a successful CTF
description: A start-to-finish guide to organizing, running, and wrapping up a capture-the-flag competition.
order: 1
---

Running a CTF takes coordination across organization, challenge development, infrastructure, and communication. These pages walk through the full lifecycle of an event, from early planning to teardown, along with the mistakes that tend to recur along the way.

::::card-grid
:::card[Prerequisites]{href=/docs/meta/running-a-successful-ctf/prerequisites}
Organizational structure, timeline, CTFtime registration, sponsorships, rules, and communication setup.
:::
:::card[Challenge design]{href=/docs/meta/running-a-successful-ctf/challenge-design}
Guidelines for planning, authoring, and testing challenges, including difficulty distribution and flag formats.
:::
:::card[Setting up a CTF platform]{href=/docs/meta/running-a-successful-ctf/setup}
Deploying rCTF, configuring Nginx with TLS, and creating admin accounts.
:::
:::card[Deploying challenges]{href=/docs/meta/running-a-successful-ctf/deployment}
Shared remotes, instanced remotes, kCTF, Docker, and admin bots.
:::
:::card[During the CTF]{href=/docs/meta/running-a-successful-ctf/during-ctf}
Monitoring, incident response, and issue resolution during an active competition.
:::
:::card[After the CTF]{href=/docs/meta/running-a-successful-ctf/after-ctf}
Uploading scoreboards to CTFtime, distributing prizes, collecting feedback, and tearing down infrastructure.
:::
:::card[Common mistakes]{href=/docs/meta/running-a-successful-ctf/common-mistakes}
Recurring operational, infrastructure, and challenge-authoring mistakes, with rCTF-specific notes.
:::
::::
