...
After installation, configure your instance by editing the YAML files in `/opt/rctf/rctf.d/` (at minimum, set `<red>ctfName</red>`, `<red>origin</red>`, `<red>startTime</red>`, and `<red>endTime</red>` - see [Configuration](/configuration/) for the full reference). Once the config is in place, apply the changes by rebuilding the stack:
