...
Or set `database.migrate: before` in your configuration to migrate on startup.
...
| `settings`       | Stores runtime-editable settings (CTF name, sponsors, etc.) |
...
V2 routes use `captchaCode` instead of `recaptchaCode` in request bodies. This applies to all endpoints that support captcha validation.
...
The v1 `recaptcha` top-level config is automatically converted to the new `captcha.provider` format at startup. However, migrating to the new format is recommended:

```yaml title="v1 (deprecated)"
recaptcha:
...
The v1 `globalSiteTag` config is automatically converted to the `analytics.provider` format. Migrate to:
...
Additional analytics providers are available: `analytics/cloudflare` and `analytics/plausible`.
