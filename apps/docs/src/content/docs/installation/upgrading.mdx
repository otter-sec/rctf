...
## tl;dr - swap the image and restart

The upgrade is automated. Point your existing deployment at the v2 image and start it - Drizzle migrations run on boot, and v1 `<red>recaptcha</red>` / `<red>globalSiteTag</red>` config is converted to the new shape at startup. No manual SQL, no config rewrite required to get back online.
...
<Callout variant="warning" title="Back up the database first">
  ...
  pg_dump`) before swapping the image so you have a path back if something goes wrong.
</Callout>

If you want to know what actually changes - new tables, new columns, API differences, and config sections that benefit from a manual rewrite - read on. None of it is required to upgrade.

## Database migration

Schema changes are handled by Drizzle ORM and run automatically when `<red>database.migrate</red>` is `<green>before</green>` (the default in the bundled container). To run them by hand:
...
All v1 routes continue to work. The v2 API is available at `<route>/api/v2/...</route>` and includes several improvements and new endpoints.
...
The v2 admin upload endpoint (`<route>POST /api/v2/admin/upload</route>`) accepts binary files via `multipart/form-data` instead of base64-encoded data URIs used in v1.
