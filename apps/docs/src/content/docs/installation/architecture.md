...
CSP is split between the SvelteKit build and container startup. SvelteKit writes `script-src{:http}` into each page as a `<meta http-equiv="Content-Security-Policy">{:html}` tag, including hashes for the generated scripts. At container startup, `$ <red>rctf</red> deployment generate-csp` reads the configured upload, captcha, and analytics providers and emits the remaining CSP directives as an nginx `Content-Security-Policy{:http}` response header. The browser enforces both policies.
...
| Directive | Sources |
| --- | --- |
| `script-src` | Generated by SvelteKit with `<green>'self'</green>`, `https://www.google.com/recaptcha/`, `https://www.gstatic.com/recaptcha/`, `https://hcaptcha.com`, `https://*.hcaptcha.com`, `https://challenges.cloudflare.com` |
