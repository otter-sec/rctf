...
| `prod-deps` | `runtime-base` | Production-only install (`--production`) on the same filter set, used as `node_modules/{:dir}` in the final image. |
...
`WORKER_EXTENSION{:sh}` is set to `<green>.js</green>` so the API process resolves its compiled worker entry (`./leaderboard.js{:file}`) rather than the `.ts` source used in development.

## Process supervision

`/etc/supervisord.conf{:file}` defines two long-running programs. Both inherit the container's PID 1 (`supervisord`), have `<red>autorestart</red>=<green>true</green>`, and stream stdout/stderr to the container's stdout/stderr without rotation.
...
| `nginx` | `nginx -g 'daemon off;'` | Serves `/app/static/{:dir}` and reverse-proxies `<route>/api</route>` and `/uploads` to `127.0.0.1:3000`. |
...
- `<red>brotli_static</red>` and `<red>gzip_static</red>` serve the `.br` and `.gz` files the SvelteKit `<red>adapter-static</red>` precompresses (`<red>precompress</red>`: `<green>true</green>` in `apps/web/svelte.config.ts{:file}`). No on-the-fly compression is configured.
...
CSP is defined in `apps/web/svelte.config.ts{:file}` and applied by SvelteKit at build time. Because `apps/web/{:dir}` is built with `adapter-static`, SvelteKit injects the policy as a `<meta http-equiv="Content-Security-Policy">` tag in each rendered page along with the auto-generated script hashes. nginx does not add a `<red>Content-Security-Policy</red>` header.
...
The `<red>instanceType</red>` config option (env: `RCTF_INSTANCE_TYPE{:sh}`) selects what an API process runs - `<green>all</green>`, `<green>frontend</green>`, or `<green>leaderboard</green>`. The dispatch happens at startup in `apps/api/src/index.ts{:file}`:
