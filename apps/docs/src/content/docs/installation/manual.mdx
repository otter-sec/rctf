...
rCTF is a Bun-ish monorepo with the following layout:
...
`WORKER_EXTENSION=.js{:sh}` makes the API load the compiled leaderboard worker from `apps/api/dist/`. The API server listens on the port specified by the `PORT{:sh}` environment variable (default `3000{:ts}`). In production, the static frontend is typically served by Nginx or a similar reverse proxy.
