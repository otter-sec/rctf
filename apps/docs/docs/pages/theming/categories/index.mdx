...
Each color has five variables defined in `app.css` for both light and dark modes: `background-l0`, `background-l1`, `background-l1-hover`, `foreground-l0`, and `foreground-l1`.
:::
...
If your category uses a color not already in the palette, add variables to `app.css` following the existing pattern. Define `--background-{color}-l0{:css}`, `--background-{color}-l1{:css}`, `--background-{color}-l1-hover{:css}`, `--foreground-{color}-l0{:css}`, and `--foreground-{color}-l1{:css}` for both `:root` and `[data-theme='dark']{:css}`. Don't forget to additionally map the colors in `@theme inline{:css}`.
