...
# Setting up a CTF platform [Deploying rCTF, configuring Nginx, and creating admin accounts]
...
### Restart rCTF after configuration changes
...
Now, we can install Nginx using `apt update && apt install -y nginx{:bash}`. Then, we create `/etc/nginx/sites-available/rctf.conf` with the following contents (note the comments on stuff you need to update):

```nginx [/etc/nginx/sites-available/rctf.conf] showLineNumbers
# Redirect HTTP -> HTTPS
server {
    listen 80;
    server_name ctf.example.com; # TODO: update me to CTF platform domain name // [!code hl]
...
Every time you update the configuration, you'll need to run `nginx -t && nginx -s reload{:bash}` to validate and apply your configuration changes. After all these steps are done, you should be able to visit `https://ctf.example.com` and view rCTF!
...
server {
    listen 443 ssl;
    server_name ctf.example.com; # TODO: update me to CTF platform domain name // [!code hl]
    ssl_certificate /etc/letsencrypt/live/ctf.example.com/fullchain.pem; # TODO: update path to CTF platform domain name // [!code hl]
    ssl_certificate_key /etc/letsencrypt/live/ctf.example.com/privkey.pem; # TODO: update path to CTF platform domain name // [!code hl]
