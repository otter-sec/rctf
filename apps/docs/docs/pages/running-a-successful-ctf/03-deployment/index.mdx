...
:::danger
**It is extremely important to make sure competitors are not able to access any internal metadata services
...
:::warning
**Note that it's possible to deploy the instance even before the CTF has started, so be careful deploying challenges ahead of time if their name is guessable.**
:::
