...
# Challenge design [Planning, authoring, and testing challenges]

## Challenge planning

Prior to authoring challenges, it is important to establish the general format and scope of the competition.

### Event format

The following are common characteristics of a well-organized CTF:

- The event lasts for either 24 or 48 hours and is held on a weekend or Friday night

- Challenges conform to standard CTF categories (web, pwn, rev, crypto, misc, forensics, OSINT, etc.)

- Each category contains approximately 2-6 challenges, with a similar number of challenges across all categories

- Challenge difficulty is distributed across a range from easy to hard, accommodating participants of varying skill levels

### Difficulty distribution

A balanced difficulty distribution ensures engagement from both beginners and experienced players. A common distribution for a category with 5 challenges might be:

| Difficulty | Count | Description |
|------------|-------|-------------|
| **Easy** | 1-2 | Solvable by beginners with good effort |
| **Medium** | 2-3 | Requires solid understanding of the category |
| **Hard** | 1-2 | Requires advanced knowledge and/or novel techniques |

### Flag format

Establish a consistent flag format before challenge development begins. Common formats include `flag{...}`, `CTF_NAME{...}`, and `ctf_name{...}`.

The flag format should be clearly communicated to participants in the rules. Consider including a regex that handles all cases, including contents within the curly braces (e.g., `CTF_name\{[\x20-\x7e]+\}`).
...
| **Hints** | Avoid actually using the "Hints" field, but rather use hints reactively during the CTF for unsolved challenges (e.g., after gauging progress at the 24-hour mark).                | Optional, uncommon        |
...
For challenges that require a remote component, containerization using Docker is strongly recommended. A typical challenge Dockerfile structure is as follows:
...
COPY --chown=challenge:challenge ./challenge /home/challenge/
...
# Expose port and run
EXPOSE 1337
CMD ["socat", "TCP-LISTEN:1337,reuseaddr,fork", "EXEC:./challenge"]
```
