...
import { createDatabase, extAuthClients } from '@rctf/db'
import {
  BadExtAuthRequest,
  BadPerms,
  BadToken,
...
const createClient = async (overrides?: {
  ...
  const res = await request(app, '/api/v2/admin/ext-auth/clients', {
    method: 'POST',
    headers: {
  ...
  const body = (await expectResponse(res, GoodAdminExtAuthClientCreate)) as {
    data: {
      id: string
      name: string
      redirectUri: string
...
const authorize = async (
  ...
  request(app, '/api/v2/ext-auth/authorize', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
const exchange = async (body: {
  ...
  request(app, '/api/v2/ext-auth/token', {
    method: 'POST',
...
describe('ext-auth happy path', () => {
  test('register client -> authorize -> token -> use against /users/me', async () => {
    ...
    const authBody = (await expectResponse(authRes, GoodExtAuthAuthorize)) as {
    ...
    const tokenBody = (await expectResponse(tokenRes, GoodExtAuthToken)) as {
  ...
  test('state with URL-unsafe characters round-trips encoded', async () => {
    ...
    const body = (await expectResponse(authRes, GoodExtAuthAuthorize)) as {
      data: { redirectTo: string }
    }
  ...
  test('client lookup returns name and redirectUri without secret', async () => {
    ...
    const res = await request(app, `/api/v2/ext-auth/clients/${client.id}`)
    const body = (await expectResponse(res, GoodExtAuthClient)) as {
...
describe('ext-auth rejection paths', () => {
  test('unknown client_id on /authorize -> badExtAuthRequest', async () => {
    const { user } = await generateRealTestUser()
    const sessionToken = await generateAuthToken(user.id)
    ...
    await expectResponse(res, BadExtAuthRequest)
  })

  test('redirect_uri mismatch on /authorize -> badExtAuthRequest', async () => {
    const { client } = await createClient({
  ...
  test('redirect_uri exact match - trailing slash and case-sensitive', async () => {
    ...
    await expectResponse(trailingSlash, BadExtAuthRequest)

    const upperCase = await authorize(sessionToken, {
      clientId: client.id,
      redirectUri: 'https://example.test/CB',
    })
    await expectResponse(upperCase, BadExtAuthRequest)
  })

  test('wrong client_secret on /token -> badExtAuthRequest', async () => {
    const { client } = await createClient()
    const { user } = await generateRealTestUser()
    const sessionToken = await generateAuthToken(user.id)
    ...
    await expectResponse(tokenRes, BadExtAuthRequest)
  })

  test('wrong client_id on /token -> badExtAuthRequest', async () => {
    const { client, secret } = await createClient()
    const { user } = await generateRealTestUser()
    const sessionToken = await generateAuthToken(user.id)
  ...
  test('expired code: code is gone from redis -> badExtAuthRequest', async () => {
    const { client, secret } = await createClient()
    const { user } = await generateRealTestUser()
    const sessionToken = await generateAuthToken(user.id)
    ...
    const redis = await createRedis()
    await redis.del(`ext-auth:code:${code}`)
  ...
  test('bogus code never issued -> badExtAuthRequest', async () => {
    const { client, secret } = await createClient()
    await expectResponse(
      await exchange({
        clientId: client.id,
  ...
  test('deleted client: pre-issued code no longer exchangeable', async () => {
    ...
    const delRes = await request(
      app,
      `/api/v2/admin/ext-auth/clients/${client.id}`,
      {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${adminToken}` },
      }
    )
    await expectResponse(delRes, GoodAdminExtAuthClientDelete)
  ...
  test('authorize requires an authenticated user', async () => {
    const { client } = await createClient()
    const res = await request(app, '/api/v2/ext-auth/authorize', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        clientId: client.id,
  ...
  test('unknown client lookup -> badExtAuthRequest', async () => {
    const res = await request(
      app,
      `/api/v2/ext-auth/clients/${crypto.randomUUID()}`
    )
    await expectResponse(res, BadExtAuthRequest)
  })
})

describe('ext-auth admin gating', () => {
  ...
  test('admin list returns created clients without secret_hash exposed', async () => {
    ...
    const body = (await expectResponse(res, GoodAdminExtAuthClients)) as {
  ...
  test('client secret is hashed at rest, never stored plaintext', async () => {
    ...
    const [row] = await db
      .select()
      .from(extAuthClients)
      .where(eq(extAuthClients.id, client.id))
