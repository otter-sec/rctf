...
import { nullsafeParseInt, nullsafeParseBoolEnv } from './util'
...
const jsonLoader = (file: string) =>
  JSON.parse(file) as PartialDeep<ServerConfig>
const yamlLoader = (file: string) =>
  yaml.parse(file) as PartialDeep<ServerConfig>

const fileConfigLoaders: Map<
  string,
  (file: string) => PartialDeep<ServerConfig>
> = new Map([
  ['json', jsonLoader],
  ['yaml', yamlLoader],
  ['yml', yamlLoader],
...
export const loadFile = (
  file: string
): PartialDeep<ServerConfig> | undefined => {
...
export const loadFileConfigs = (
  configPath?: string
): PartialDeep<ServerConfig>[] => {
  const resolvedConfigPath =
    configPath ?? process.env.RCTF_CONF_PATH ?? findConfigDir()

  return fs
    .readdirSync(resolvedConfigPath)
...
export const loadEnvConfig = (): PartialDeep<ServerConfig> =>
  removeUndefined({
    database: {
    ...
    leaderboard: {
      ...
      updateInterval: nullsafeParseInt(
        process.env.RCTF_LEADERBOARD_UPDATE_INTERVAL
      ),
      graphMaxTeams: nullsafeParseInt(
        process.env.RCTF_LEADERBOARD_GRAPH_MAX_TEAMS
      ),
      graphSampleTime: nullsafeParseInt(
        process.env.RCTF_LEADERBOARD_GRAPH_SAMPLE_TIME
      ),
    },
